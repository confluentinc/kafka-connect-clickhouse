...
// Strip leading "v" so the version is a valid Maven coord (e.g. "v1.3.0" -> "1.3.0").
// The cloud module consumes this artifact via maven-install-plugin; the coordinate
// must match <clickhouse.kafka.version> in kafka-connect-clickhouse-cloud/pom.xml.
...
// Publish the shaded `confluentJar` so kafka-connect-clickhouse-cloud can resolve it
// via Maven coordinates (com.clickhouse.kafka:clickhouse-kafka-connect:<version>:confluent).
// `gradle publishToMavenLocal` installs into ~/.m2; the release script in the parent
// repo invokes this before running `mvn verify` on the cloud module.
publishing {
    publications {
